<?php foreach($_SESSION["task"]["first_subtasks"]["complex_numbers"] as $complex_number_counter => $complex_number):?>
    ...
    <div class="small_task_container">
        ...
        ?>
        <?php 
            $solution_label = "<label>Im(v<sub>$bottom_index</sub>) = </label>";
        ...
        ?>
        <?php
            $solution_label = "<label style=\"text-decoration: overline;margin:0%\" >v</label><sub>$bottom_index</sub><label style=\"margin-right:2%\"> = </label>"; 
        ...
        <label class="task_label">
            ...
            <?php foreach($quadrupel_of_complex_numbers as $complex_number_name => $complex_number):?>
                <?php if($complex_number_counter !== 0):?>
                    , 
                <?php endif?>
                <?= PrintServices::CreatePrintableComplexNumberAlgebraic("$complex_number_name<sub>$bottom_index</sub>", $complex_number)?>
    ...
    <?php foreach($operations as $operation_name => $operation):?>
        ...
        <div class="small_task_container">
            ...
            <?php if($operation_name === "addition"):?>
                <?php 
                    $solution_label = "<label >" . $operands[0] . "<sub>$bottom_index</sub> + " . $operands[1] . "<sub>$bottom_index</sub> = </label>";
                ?>
