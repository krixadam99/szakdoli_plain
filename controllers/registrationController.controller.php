        ...
        private function EmailAddressValidator() : void {
            ...
            if(trim($user_email)!="E-mail cím" && trim($user_email)!= ""){
                if(!filter_var($user_email, FILTER_VALIDATE_EMAIL)){//Wrong format for the email address
                    $this->SetIncorrectParameter("wrong_2_wrong_format");
                }else{
                    $this->SetCorrectParameter("user_email", $user_email);
                }
            }else{//Email address is not given
                $this->SetIncorrectParameter("wrong_2_no_email");
            }
        ...
        private function SubjectNameValidator() : void {
            ...
            if(isset($subject_name)){
                if($subject_name == "Diszkrét matematika I."){
                    ...
                    $this->SetCorrectParameter("subject_name", "0");
                ...
                $this->SetIncorrectParameter("wrong_3_no_subject_data");
            }
        }
        ...
        private function UserStatusValidator() : void {
            ...
            if(isset($user_status)){
                if($user_status == "Diák" || $user_status == "Demonstrátor"){
                    $this->SetCorrectParameter("user_status", $user_status);
                }else{
                    $this->SetIncorrectParameter("wrong_4_no_such_status");      
                }
            }else{//No subject name given
                $this->SetIncorrectParameter("wrong_4_no_user_status");
            }
        }
            ...
            if(isset($subject_group)){
                if($user_status == "Diák"){
                    if($subject_name == "dimat_i" || $subject_name == "dimat_ii"){
                        ...
                        if($is_correct){
                            $this->SetCorrectParameter("subject_group", $subject_group);
                        }else{
                            $this->SetIncorrectParameter("wrong_5_no_such_group");
                        }
                    }else{
                        $this->SetCorrectParameter("subject_group", 0);
                    }
                }else{
                    $this->SetCorrectParameter("subject_group", $subject_group);
                }
            }else{//No subject name given
                $this->SetIncorrectParameter("wrong_5_no_subject_group");
            }
        }
            ...
            if(isset($user_password) && trim($user_password)!="Jelszó"){
                if(strlen($user_password) >= 8){
                    if(!preg_match("/[a-z]/", $user_password) || !preg_match("/[A-Z]/", $user_password) || !preg_match("/[0-9]/", $user_password) || !preg_match("/[\-\,\.\?\!]/", $user_password)){
                        $this->SetIncorrectParameter("wrong_6_complexity");
                    }
                }else{
                    $this->SetIncorrectParameter("wrong_6_length");
                }
            }else{
                $this->SetIncorrectParameter("wrong_6_no_password");
            }
        }
        ...
        private function PasswordAgainValidator() : void {
            ...
            if(isset($user_password_again) && trim($user_password_again)!="Jelszó megerősítése"){
                if($user_password != $user_password_again){
                    $this->SetIncorrectParameter("wrong_7_not_same");
                }
            }else{
                $this->SetIncorrectParameter("wrong_7_no_password_validate");
            }
        }
