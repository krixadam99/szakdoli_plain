    ...
    */
    class PracticeModel extends TasksModel {
        ...
        public function UpdatePracticeScore($neptun_code, $practice_number, $previous_point, $update_point){
            ...
            $query = "BEGIN; UPDATE practice_task_points SET task_point =  $new_point WHERE ";
            $query .= "subject_group_id = (SELECT subject_group_id FROM user_status JOIN subject_groups USING(subject_group_id) WHERE user_status.application_request_status = \"APPROVED\" AND user_status.is_teacher = \"0\" AND user_status.neptun_code = \"$neptun_code\")";
            $query .= " AND neptun_code = \"$neptun_code\" AND task_type = \"practice_task_$practice_number \"; ";
            ...
            if($previous_point < 10 && $new_point >= 10){
                //$extra_points = $this->database->LoadDataFromDatabase("SELECT extra FROM results WHERE subject_group_id = (SELECT subject_group_id FROM user_status JOIN subject_groups USING(subject_group_id) WHERE user_status.application_request_status = \"APPROVED\" AND user_status.is_teacher = \"0\" AND user_status.neptun_code = \"$neptun_code\")");
                $extra_points = $this->database->LoadDataFromDatabaseWithPDO("SELECT extra FROM results WHERE subject_group_id = (SELECT subject_group_id FROM user_status JOIN subject_groups USING(subject_group_id) WHERE user_status.application_request_status = \"APPROVED\" AND user_status.is_teacher = \"0\" AND user_status.neptun_code = \"$neptun_code\")");
                $extra_points = $extra_points[0]??array("extra" => 0);
                $extra_points = $extra_points["extra"];

                $current_date = date("Y-m-d");
                //$practice_task_due_date = $this->database->LoadDataFromDatabase("SELECT due_to FROM task_due_to_date_table WHERE task_type = \"practice_task_$practice_number\" AND subject_group_id = (SELECT subject_group_id FROM user_status JOIN subject_groups USING(subject_group_id) WHERE user_status.application_request_status = \"APPROVED\" AND user_status.is_teacher = \"0\" AND user_status.neptun_code = \"$neptun_code\")");
                $practice_task_due_date = $this->database->LoadDataFromDatabaseWithPDO("SELECT due_to FROM task_due_to_date_table WHERE task_type = \"practice_task_$practice_number\" AND subject_group_id = (SELECT subject_group_id FROM user_status JOIN subject_groups USING(subject_group_id) WHERE user_status.application_request_status = \"APPROVED\" AND user_status.is_teacher = \"0\" AND user_status.neptun_code = \"$neptun_code\")");
                ...
                if($current_date <= $practice_task_due_date){   
                    $query .= "UPDATE results SET result = $extra_points + 1 WHERE ";
                    $query .= "subject_group_id = (SELECT subject_group_id FROM user_status JOIN subject_groups USING(subject_group_id) WHERE user_status.application_request_status = \"APPROVED\" AND user_status.is_teacher = \"0\" AND user_status.neptun_code = \"$neptun_code\")";    
                    $query .= " AND neptun_code = \"$neptun_code\" AND task_type =  \"extra\"; ";
                }
            }
            $query .= "COMMIT";
